...
# ============================================================
# SAFE PNG EXPORT HELPERS
# ============================================================

safe_export_number <- function(x, default_value, min_value = NULL, max_value = NULL) {
...
save_ggplot_download <- function(file, plot_object, width, height, dpi, bg = "white") {
  ...
  tryCatch({
    ...
    write_error_png(file, conditionMessage(e), width = width, height = height, dpi = min(dpi, 600), bg = bg)
  }, finally = {
    if (file.exists(tmp_png)) unlink(tmp_png)
  })
}

save_base_png_download <- function(file, plot_function, width, height, dpi, bg = "white") {
  width <- safe_export_number(width, 8, 3, 20)
  height <- safe_export_number(height, 6, 3, 20)
  dpi <- safe_export_number(dpi, 1200, 72, 1500)
  tmp_png <- tempfile(fileext = ".png")
  ...
  tryCatch({
    ...
    # Close any open graphics device if the export failed while plotting.
